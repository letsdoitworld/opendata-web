...
class App extends Component {
  ...
  static get defaultProps() {
    return {
      ...
      apiURL: "https://wwpapi.letsdoitworld.org/api",
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      updatedCountry: {},
      updatedTrashPoint: {},
      aboutClassName: this.isAboutInfoRequired(props)
        ? "about-shown"
        : "hidden",
      downloadClassName: this.isDownloadPanelRequired(props)
        ? "about-shown"
        : "hidden",
      getInvolvedClassName: this.isGetInvolvedRequired(props)
        ? "about-shown"
        : "hidden",
      readMoreClassName: this.isReadMoreRequired(props)
        ? "about-shown"
        : "hidden"
    };
  ...
  async componentDidMount() {
    ...
    if (this.isTrashPointDetailsRequired(this.props)) {
      ...
      const countryCode = this.props.location.pathname.substring(
    ...
    return await fetch(this.props.apiURL + endpoint)
      ...
      .then(data => {
        if (data.status === "SUCCESS") {
          ...
          console.error(
            "endpoint " + endpoint + " failed " + JSON.stringify(data)
          );
        }
      })
  ...
  async loadReportsCounter() {
    this.setState({
      reportsCounter: await this.loadData("/reports/count", data =>
        parseInt(data.reports_count)
      )
    });
  }

  async loadTrashPointDetails(props) {
    const trashPointId = props.location.pathname.substring(
      props.location.pathname.lastIndexOf("/") + 1
    );
    this.setState({
      selectedTrashPoint: await this.loadData(
        "/trashpoint/" + trashPointId,
        data => data.sources[0]
      )
    });
  }
  ...
  async loadCountryDetails(countryCode) {
    let clickedCountry = this.state.allCountries.find(
      o => o.code.toLowerCase() == countryCode
    );
    this.setState({
      selectedCountry: clickedCountry,
      updatedCountry: clickedCountry
    });
  }
  ...
  async updateTrashpointsData(props) {
    ...
    this.setState({
      updatedTrashPoint: this.state.selectedTrashPoint,
      updatedCountry: this.state.selectedCountry
    });
  }
  ...
  async loadCountriesData() {
    ...
    const allCountries = await this.loadData("/countries", data =>
      data.sources.map(
        key =>
          new Country(
  ...
  async componentWillReceiveProps(nextProps) {
    ...
    if (this.isTrashPointDetailsRequired(nextProps)) {
      ...
      const countryCode = nextProps.location.pathname.substring(
  ...
  cartoClient = new carto.Client({
    apiKey: this.props.cartoApiKey,
    username: this.props.cartoUsername
  });
  ...
  render() {
    const LeftPanel = () => (
      <Switch>
        <Route
          exact
          path={"/(|about|download|getinvolved|readmore)"}
          component={IntroText}
        />
        <Route
          exact={false}
        ...
        {this.state.countriesDataLoaded && (
          <div className="app-wrapper">
            ...
            <Download
              ...
              allCountries={this.state.allCountries.sort((key, key1) =>
                key.name.localeCompare(key1.name)
              )}
              apiURL={this.props.apiURL}
